...
#    pres, temp, xo, xmg, xsi = x
#    lnGamma_O = -oo1*1873/temp*np.log(1-xo) - (omg1*1873/temp*xmg*(1+np.log(1-xmg)/xmg-1/(1-xo)) + osi1*1873/temp*xsi*(1+np.log(1-xsi)/xsi-1/(1-xo))) + (omg1*1873/temp*xmg**2*xo*(1/(1-xo)+1/(1-xmg)+xo/(2*(1-xo)**2)-1) + osi1*1873/temp*xsi**2*xo*(1/(1-xo)+1/(1-xsi)+xo/(2*(1-xo)**2)-1))
#    lnKsi = a41 + b41/temp + c41*pres/temp - 2*lnGamma_O - (-sisi1*1873/temp*np.log(1-xsi) - (osi1*1873/temp*xo*(1+np.log(1-xo)/xo-1/(1-xsi)) + mgsi1*1873/temp*xmg*(1+np.log(1-xmg)/xmg-1/(1-xsi))) + (osi1*1873/temp*xo**2*xsi*(1/(1-xsi)+1/(1-xo)+xsi/(2*(1-xsi)**2)-1) + mgsi1*1873/temp*xmg**2*xsi*(1/(1-xsi)+1/(1-xmg)+xsi/(2*(1-xsi)**2)-1)))
#    return lnKsi
...
def equation4(x, a1, b1, c1, hh, ho, hmg, hsi, oo, omg, osi):
    ...
    lnKh = a1 + b1/temp + c1*pres/temp - lnGamma_O - 2*(-hh*1873/temp*np.log(1-xh) - (ho*1873/temp*xo*(1+np.log(1-xo)/xo-1/(1-xh)) + hmg*1873/temp*xmg*(1+np.log(1-xmg)/xmg-1/(1-xh)) + hsi*1873/temp*xsi*(1+np.log(1-xsi)/xsi-1/(1-xsi))) + (ho*1873/temp*xo**2*xh*(1/(1-xh)+1/(1-xo)+xh/(2*(1-xh)**2)-1) + hmg*1873/temp*xmg**2*xh*(1/(1-xh)+1/(1-xmg)+xh/(2*(1-xh)**2)-1) + hsi*1873/temp*xsi**2*xh*(1/(1-xh)+1/(1-xsi)+xh/(2*(1-xh)**2)-1)))
    return lnKh
...
data_2 = pd.read_excel('HOMgSiFe.xlsx')
...
params.add('ho', value=0.0, vary=False)
params.add('hmg', value=0.0, vary=False)
params.add('hsi', value=0.0)
#params.add('oo', value=-5.0, vary=False)
params.add('oo', value=0.0)
