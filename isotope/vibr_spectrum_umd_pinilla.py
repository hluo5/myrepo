...
#    start_time  = time.time()
    ...
    Boltzmann=8.6173303*10**(-5) #eV*K^-1
    Avogadro=6.022140857*(10**(23)) # mol-1
    au_angstrom_squre = 1.0 / Avogadro * (10**(-3)) * (10**10) / (1.602176634 * 10**-19) # unit is eV, (g/mol)*(Angstrom^2/fs^2) transform to Kg*m^2/s^2, then transform to eV.
    kB_T = temperature * Boltzmann # eV
    ...
    # average over species, and no mass weighted
    ...
    temp1 = total_average_correlation[0]
    total_average_correlation[:] = total_average_correlation[:] / temp1
    temp2 = np.copy(average_correlation[0]) #normalization
    for ii in range(MyCrystal.ntypat):
        average_correlation[:,ii] = average_correlation[:,ii] / temp2[ii]
...
#    diffusion_coefficient = np.zeros(MyCrystal.ntypat)
    ...
    for ii in range(MyCrystal.ntypat):
        average_fft_correlation[:,ii] = average_fft_correlation[:,ii] / temp2[ii]
