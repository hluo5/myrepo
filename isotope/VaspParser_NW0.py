...
def read_outcar(FileName,InitialStep):
    ...
    switch = False
    flagrprimd = -1 #length scales by which dimensionless primitive translations
    iatom = -1    
    istep = 0
    MyCrystal = cr.Lattice()
    TimeStep = 1.0
    newfile = 'output.umd.dat'
    nf = open(newfile,'w')
    nf.close()
    atomictype = 0
    CurrentTime = 0.0
    TimeStep = 0.0
    flagscalee = -1
    with open(FileName,'r') as ff:           # using "with open" so that file is automatically closed once exits
        while True:
            line = ff.readline()
            if not line: break
            line=line.strip()                 # remove characters from the start and end
            entry=line.split()                # return a list seperated by the delimiter
            if (len(entry)>0):
                if (entry[0]=='ions'):          #determine how many atoms, read the line of "ions per type"
                    jatom = 0
                    MyCrystal.natom = 0
                    MyCrystal.ntypat = len(entry)-4   #determine how many different types of atoms
                    MyCrystal.types = [0 for _ in range(MyCrystal.ntypat)] #use underscore(_) as a variable in looping
                    for ii in range(len(entry)-4):
                        MyCrystal.natom = MyCrystal.natom + int(entry[4+ii]) # determine total number of atoms
                        MyCrystal.types[ii] = int(entry[4+ii])               # determine how many atoms per type
                    
                    MyCrystal.typat = [0 for _ in range (MyCrystal.natom)]   #gives the type of each atom 
                    for ii in range(len(entry)-4):
                        atomictype += 1
                        for jj in range(int(entry[4+ii])):
                            MyCrystal.typat[jatom] = atomictype-1
                            jatom = jatom + 1
                    MyCrystal.atoms = [cr.Atom() for _ in range(MyCrystal.natom)] #Atom is a class, producing a object which inherit variable and function.
                    oldpos = [cr.Atom() for _ in range(MyCrystal.natom)]           #Atom is a class, producing a object which inherit variable and function.
                    ...
                    break                        # jump out of cycle after getting basic information prepared
    ff.close()
    flagmass = 0
    flagtitle = 0
    atn = ''                                    # atomic name
            ...
            if (len(entry)>0):
                if (len(entry)>1):
                    if entry[1] == 'Iteration':
                        break                  # jump out of cycle before reaching the true loop
#                if (entry[0]=='VRHFIN'):
#                    flagtitle += 1
...
#                    print ('just chekcing: ',atn,ats,atno)
                if (entry[0]=='POTCAR:'):
                    if (flagtitle < MyCrystal.ntypat) :
                        MyCrystal.elements[flagtitle] = entry[2].split('_')[0]                  # read elements
#                        print ('element ',flagtitle,' is ',MyCrystal.elements[flagtitle])     
                        (atn,ats,atno,MyCrystal.masses[flagtitle])=cr.Elements2rest(MyCrystal.elements[flagtitle])  # read mass from a given list
#                        print ('just chekcing: ',atn,ats,atno)
...
#                        for ii in range(MyCrystal.ntypat):
                        MyCrystal.masses[flagmass] = float(entry[2][:-1])                        # read mass from POMASS
#                        MyCrystal.zelec[flagmass] = float(entry[5])
                        flagmass += 1
                if (entry[0]=='ZVAL'):                                                           # read ionic valenz
                    for ii in range(MyCrystal.ntypat):
...
#                    flagmass=1
    ...
    with open(FileName,'r') as ff:
        while True:
            ...
            line=line.strip()
            entry=line.split()
            if (len(entry)>=4 and entry[1]=='Iteration'):
                    if (entry[3]=='1)'):
                        iatom = 0
            if (iatom>-1):                  #reading the atomic positions, will read these information even if has not reached the InitialStep, but won't calculate velocities & diffcoord etc.
...
#                    with open("XDATCAR",'r') as xx:
#                        xxx = xx.readlines()[istep*MyCrystal.natom+iatom]
                    xx = istep*MyCrystal.natom+iatom+1
                    xxx = linecache.getline('XDATCAR',xx)      # this way is much faster than readlines.
                    xxx=xxx.strip()
                    entry=xxx.split()
                    MyCrystal.atoms[iatom].xcart = [MyCrystal.acell[0]*float(entry[0]),MyCrystal.acell[1]*float(entry[1]),MyCrystal.acell[2]*float(entry[2])]
...
#                    print ('the line of 6 elements: ',entry)
            ...
            if(switch==False):
                if (len(entry)>0 and entry[0]=='POTIM'):
                    TimeStep=float(entry[2])                                                   # read timestep
                if (len(entry)>=2 and entry[1]=='aborting'):                                   # aborting means one ionic step is over. Then can read calculated results.
                    switch = True
                else:
                    continue
            if (switch==True and len(entry)>0):                                               # start reading calculated results (switch = True)
...
#                        MyCrystal.internalenergy=float(entry[3])  #this leads only part of the internal energy, one should add the kinetic energy of ions
                                                                  #the variance of {this energy + kinetic energy of ions}  yields Cv, which can be expressed as (dU/dT)v
                        flagscalee += 1
